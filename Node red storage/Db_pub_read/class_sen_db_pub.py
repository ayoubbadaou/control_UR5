...
class SenDbToOPCUA:
    def __init__(self):
        ...
        self.opc_endpoint = "opc.tcp://0.0.0.0:3016/sen_data/"
        self.namespace_uri = "UR5.SENData"
    ...
    def initialize_opcua_server(self):
        ...
        table_node = objects.add_object(idx, "SENPoseTable")
        ...
        self.column_nodes = {
            ...
            "conv1": table_node.add_variable(idx, "CONV1", [], varianttype=ua.VariantType.Double),
            "conv2": table_node.add_variable(idx, "CONV2", [], varianttype=ua.VariantType.Double),
            "bac1": table_node.add_variable(idx, "BAC1", [], varianttype=ua.VariantType.Double),
            "bac2": table_node.add_variable(idx, "BAC2", [], varianttype=ua.VariantType.Double),
            "bac3": table_node.add_variable(idx, "BAC3", [], varianttype=ua.VariantType.Double),
            "bac4": table_node.add_variable(idx, "BAC4", [], varianttype=ua.VariantType.Double),
            "bac5": table_node.add_variable(idx, "BAC5", [], varianttype=ua.VariantType.Double)
        }
        ...
        if current_last_id > self.last_row_node.get_value():
            ...
            if new_rows:
                ...
                for row in new_rows:
                    ...
                    self.column_nodes["conv1"].set_value(self.column_nodes["conv1"].get_value() + [float(row[2] or 0.0)])
                    self.column_nodes["conv2"].set_value(self.column_nodes["conv2"].get_value() + [float(row[3] or 0.0)])
                    self.column_nodes["bac1"].set_value(self.column_nodes["bac1"].get_value() + [float(row[4] or 0.0)])
                    self.column_nodes["bac2"].set_value(self.column_nodes["bac2"].get_value() + [float(row[5] or 0.0)])
                    self.column_nodes["bac3"].set_value(self.column_nodes["bac3"].get_value() + [float(row[6] or 0.0)])
                    self.column_nodes["bac4"].set_value(self.column_nodes["bac4"].get_value() + [float(row[7] or 0.0)])
                    self.column_nodes["bac5"].set_value(self.column_nodes["bac5"].get_value() + [float(row[7] or 0.0)])
